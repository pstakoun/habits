...
export default class Habit extends Component {
	...
	render() {
		...
		for (var i = Math.floor((today - this.props.habit.createdAt) / (1000* 60 * 60 * 24)); i >= -1; i--) {
			chartData.labels.push('');
			var count = 0;
			for (var j = 0; j < this.props.habit.datesCompleted.length; j++) {
				console.log();
				if (Math.floor((today - new Date(this.props.habit.datesCompleted[j].substring(0, 4)+'-'+this.props.habit.datesCompleted[j].substring(4, 6)+'-'+this.props.habit.datesCompleted[j].substring(6,8))) / (1000* 60 * 60 * 24)) > i) {
					count++;
				}
			}
